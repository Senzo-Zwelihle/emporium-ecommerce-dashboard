...
import { FeedbackManager } from "@/components/admin/feedback/feedback-manager"
...
export default async function ExperienceDetailPage({
  ...
  const handleUpdateStatus = async (experienceId: string, status: string) => {
    ...
    console.log(`Updating experience ${experienceId} status to ${status}`)
  }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage and respond to customer user experiences
          </p>
        </div>
        <Separator />
        <FeedbackManager
