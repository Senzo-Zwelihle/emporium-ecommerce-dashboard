...
const FavoriteCard = ({
  ...
  favorite: UserAccountData["favorites"][0]
}) => {
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5].map(star => (
              <StarIcon
                key={star}
                className={`size-4 ${
                  star <= 5 ? "fill-yellow-400 text-yellow-400" : ""
                }`}
              />
            ))}
          </div>
...
export const FavoritesTab = ({ favorites }: FavoritesTabProps) => {
  ...
  const totalValue = favorites.reduce(
    (sum, fav) => sum + Number(fav.product.price),
    0
  )
        ...
        <CardContent className="flex flex-col items-center justify-center space-y-6 py-16">
          ...
          <div className="space-y-2 text-center">
            ...
            <p className="mx-auto max-w-sm">
              Save products you love! Favorited items will appear here for easy
              access.
            </p>
          </div>
