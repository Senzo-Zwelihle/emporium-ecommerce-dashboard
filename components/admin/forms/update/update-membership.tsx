...
import { PlusIcon } from "@/components/pqoqubbw/icons/plus"
import { XIcon } from "@/components/pqoqubbw/icons/x"
...
} from "@/schemas/admin/membership"
import { updateMembershipAction } from "@/server/actions/admin/membership"
...
const UpdateMembershipForm = ({ data }: UpdateMembershipFormProps) => {
  // Form states
  ...
  const handleRemoveBenefit = (benefitToRemove: string) => {
    ...
    // If the schema requires at least one benefit, add a manual error if array becomes empty
    if (
  ...
  function onSubmit(values: MembershipSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        updateMembershipAction(values, data.id)
      )
  ...
  return (
    <Form {...form}>
      <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Give the membership a short, descriptive name.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormItem>
          ...
          <div className="flex items-center space-x-2">
            <Input
              ...
              className={cn(
                form.formState.errors.benefits && "border-destructive"
              )}
            />
        ...
        {/* Minimum Points */}
        <FormField
          control={form.control} // Keep using form.control
          name="minPoints"
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  {...field}
                  onChange={e => {
                    // Convert input value to a number; handle empty string
                    field.onChange(
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                The maximum points for this membership. Set to 0 if there&apos;s
                no upper limit.
              </FormDescription>
              <FormMessage />
            </FormItem>
