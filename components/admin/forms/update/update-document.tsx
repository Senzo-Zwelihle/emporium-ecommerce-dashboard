...
const UpdateDocumentForm = ({ document }: UpdateDocumentFormProps) => {
  // Form states
  ...
  const form = useForm<DocumentSchemaType>({
    ...
    defaultValues: {
      name: document.name,
      documentType: document.documentType as DocumentSchemaType["documentType"],
      status: document.status as DocumentSchemaType["status"],
      file: document.file,
  ...
  function onSubmit(values: DocumentSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        updateDocumentAction(values, document.id)
      )
