...
const UpdateNoteForm = ({ note }: UpdateNoteFormProps) => {
  ...
  const form = useForm<NoteSchemaType>({
    ...
    defaultValues: {
      ...
      tag: note.tag as NoteSchemaType["tag"],
      status: note.status as NoteSchemaType["status"],
      action: note.action as NoteSchemaType["action"],
  ...
  function onSubmit(values: NoteSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        updateNoteAction(values, note.id)
      )
      ...
      if (result.status === "success") {
        ...
        form.reset()
        router.push("/admin/documents/notes")
