...
import { updateSwatchAction } from "@/server/actions/admin/swatch"
...
interface UpdateSwatchFormProps {
  swatch: {
    id: string
    productId: string
...
const UpdateSwatchForm = ({ swatch }: UpdateSwatchFormProps) => {
  ...
  function onSubmit(values: SwatchSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        updateSwatchAction(values, swatch.id)
      )
      ...
      if (result.status === "success") {
        ...
        router.push(`/admin/products/${swatch.productId}`)
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Upload images for this variant option
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
...
export default UpdateSwatchForm
