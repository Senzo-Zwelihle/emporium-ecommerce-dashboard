...
import { warehouseSchema, WarehouseSchemaType } from "@/schemas/admin/warehouse"
import { WarehouseStatus } from "@/lib/generated/prisma"
import { updateWarehouseAction } from "@/server/actions/admin/warehouse"
...
const UpdateWarehouseForm = ({ data }: UpdateWarehouseFormProps) => {
  // Form states
  ...
  const form = useForm<WarehouseSchemaType>({
    ...
    defaultValues: {
      ...
      status: data.status as WarehouseSchemaType["status"]
    }
  })
  ...
  function onSubmit(values: WarehouseSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        updateWarehouseAction(values, data.id)
      )
